      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-8">
          <div className="mb-6 flex items-center gap-3">
            ...
            <span
              className={`text-xs px-2 py-1 rounded-full ${status === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
            >
          ...
          <div className="bg-white rounded-xl lg:rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col mb-6">
            <div className="bg-gray-50 px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${status === "active" ? "bg-green-500 animate-pulse" : "bg-gray-400"}`}
                ></div>
            ...
            <div className="relative bg-gray-900 aspect-video lg:h-[450px] flex items-center justify-center overflow-hidden">
              {status === "error" && (
                <p className="text-red-400 text-sm px-4">Kesalahan: {error}</p>
              )}
              ...
              <video
                ...
                playsInline
                muted
                className={`w-full h-full object-cover transition-opacity duration-300 ${status === "active" ? "opacity-100" : "opacity-0 absolute"}`}
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl border-l-4 border-blue-500 shadow-sm p-6 relative">
              ...
              <p
                className={`text-xl font-medium leading-relaxed min-h-[80px] ${isProcessing ? "text-blue-400 italic" : "text-gray-900"}`}
              >
            ...
            <div className="bg-white rounded-xl border-l-4 border-green-500 shadow-sm p-6 relative">
              ...
              {speechError && (
                <p className="text-sm text-red-500 mb-2">Browser tidak mendukung Web Speech API.</p>
              )}

              <p
                className={`text-xl font-medium leading-relaxed min-h-[80px] ${!transcript ? "text-gray-400 italic" : "text-gray-700"}`}
              >
